...
def saveFrameAsImage(surface: pygame.surface) -> None:
    """When called, the current frame will be saved"""
    fileName = 'assigned_image_for_101227727.png'
...
while True: # Loop through infinently
    ...
    for event in pygame.event.get(): 
        if event.type == pygame.QUIT:
            saveFrameAsImage(window) # Save the last frame as image (In this case only frame)
            pygame.quit()
